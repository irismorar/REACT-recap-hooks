...
if it is "on" it becomes "off" if it is "off" it becomes "on", but only if making it "on" does not result in more than the maximum number of buttons (3) being "on"! If making it on would result in more than three buttons being "on" then the button stays "off". The buttons that are "on" have a background color of blue. The buttons that are "off" have a backgound color of gray.
